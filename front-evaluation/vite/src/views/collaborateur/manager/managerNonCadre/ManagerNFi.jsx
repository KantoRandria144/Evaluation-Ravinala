...
function ManagerNFi({ subordinateId, typeUser }) {
  ...
  const validateFinalObjectif = async () => {
    try {
      ...
      // Supprimer la vérification de signature obligatoire
      // if (!signatureFile) {
      ...
      // Vérifier et comparer la signature seulement si elle est fournie
      if (signatureFile) {
      ...
      const updatePromises = results.map(async (indicator) => {
        ...
        const response = await formulaireInstance.post(
          `Evaluation/UpdateUserIndicatorResults?userId=${subordoneId}&userIndicatorId=${userIndicatorId}`,
          updatedResults
        );
      ...
      const helpContents = helps.map((help) => ({
        ...
        type: type,
        helpId: help.helpId,
        content: helpInputs[help.helpId] || 'Aucun'
      }));
          ...
          <Box sx={{ marginTop: 3 }}>
            ...
            <Box sx={{ display: 'flex', justifyContent: 'space-between', mt: 2 }}>
              ...
              {activeStep === steps.length - 1 ? (
                ...
                >
                  {isValidate ? 'Déjà validé' : isLoading ? 'En cours...' : 'Valider'}
                </Button>
              ) : (
                <Button
                  variant="contained"
