...
import { Card, CardContent, Typography, CircularProgress, useMediaQuery, useTheme } from '@mui/material';
...
function ScoreChart({ userId, phase }) {
  ...
  const width = isMdUp ? 1100 : 600;
  const height = isMdUp ? 300 : 200;

  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await formulaireInstance.get(`/Stat/getFinaleScoreNonCadre/${userId}/${phase}`);
